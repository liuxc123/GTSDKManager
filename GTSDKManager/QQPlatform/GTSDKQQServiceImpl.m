...
@interface GTSDKQQServiceImpl () <TencentSessionDelegate, QQApiInterfaceDelegate> {
    BOOL isLogining;
    NSError *error;
    TencentOAuth *tencentOAuth;
    void (^MyBlock)(NSDictionary *oauthInfo, NSDictionary *userInfo, NSError *qqerror);
    GTSDKQQCallbackBlock shareBlock;
}
...
@implementation GTSDKQQServiceImpl

+ (instancetype)sharedService
{
    static GTSDKQQServiceImpl *sharedInstance = nil;
...
- (void)registerWithPlatformConfig:(NSDictionary *)config
{
    ...
    NSString *qqAppId = config[GTSDKConfigAppIdKey];
              ...
              onComplete:(GTSDKShareCallback)complete
{
    ...
    if (urlString) {  //链接分享
        ...
        if (contentData.length > 5000 * 1024) {  //图片不能超过5M
            ...
            contentImage = [contentImage GTSDKShare_resizedImage:contentSize
                                            interpolationQuality:kCGInterpolationDefault];
...
- (void)handleShareResultInActivity:(id)result onComplete:(GTSDKShareCallback)complete
{
...
- (QQApiSendResultCode)sendReq:(QQBaseReq *)req
                   shareModule:(NSUInteger)shareModule
                      callback:(GTSDKQQCallbackBlock)callbackBlock
{
