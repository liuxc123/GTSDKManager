//
//  GTSDKWeiboServiceImpl.m
//  FBSnapshotTestCase
//
//  Created by liuxc on 2018/11/15.
//

#import "GTSDKWeiboServiceImpl.h"
#import "WeiboSDK.h"

typedef void (^LDWeiboCallbackBlock)(WBBaseResponse *resp);

@interface GTSDKWeiboServiceImpl () <WeiboSDKDelegate> {
    BOOL isRegistered;
    NSString *shareText;
    UIImage *shareImage;
...
@implementation GTSDKWeiboServiceImpl

+ (instancetype)sharedService
{
    static GTSDKWeiboServiceImpl *sharedInstance = nil;
...
- (void)registerWithPlatformConfig:(NSDictionary *)config
{
    ...
    isRegistered = NO;
    NSString *appid = config[GTSDKConfigAppIdKey];
              ...
              onComplete:(GTSDKShareCallback)complete
{
    ...
    redirectURI = content[GTSDKShareContentRedirectURIKey];
    shareText = content[GTSDKShareContentTextKey];
    shareImage = content[GTSDKShareContentImageKey];
