//
//  GTSDKWXServiceImpl.m
//  FBSnapshotTestCase
//
//  Created by liuxc on 2018/11/15.
//

#import "GTSDKWXServiceImpl.h"
#import "WXApi.h"
#import "NSString+GTSDKAdditions.h"
#import "NSDictionary+GTSDKAdditions.h"
...
@interface GTSDKWXServiceImpl () <WXApiDelegate, NSURLConnectionDataDelegate> {
    NSDictionary *oauthDict;
    void (^MyBlock)(NSDictionary *oauthInfo, NSDictionary *userInfo, NSError *wxerror);

    GTSDKWXCallbackBlock wxcallbackBlock;

    BOOL _shouldHandleWXPay;
    GTSDKPayCallback _wxCallback;
}
...
@implementation GTSDKWXServiceImpl


+ (instancetype)sharedService
{
    static GTSDKWXServiceImpl *sharedInstance = nil;
...
- (void)registerWithPlatformConfig:(NSDictionary *)config
{
    ...
    NSString *wxAppId = config[GTSDKConfigAppIdKey];
    NSString *wxAppSecret = config[GTSDKConfigAppSecretKey];
    NSString *wxDescription = config[GTSDKConfigAppDescriptionKey];
...
- (void)wxpayOrder:(NSString *)orderString callback:(GTSDKPayCallback)callback
{
...
- (BOOL)sendReq:(BaseReq *)req callback:(GTSDKWXCallbackBlock)callbackBlock
{
                                ...
                                parameters:(NSDictionary *)parameters
{
    ...
    if (parameters) {
       ...
       forHTTPHeaderField:@"Content-Type"];
        [request setHTTPBody:[[self GTSDKQueryStringFromParametersWithEncoding:parameters
...
- (NSString *)GTSDKQueryStringFromParametersWithEncoding:(NSDictionary *)parameters
                                                encoding:(NSStringEncoding)stringEncoding
{
    ...
    for (NSString *key in [parameters allKeys]) {
        ...
        if (!value || [value isEqual:[NSNull null]]) {
            [mutablePairs addObject:GTSDKAFPercentEscapedQueryStringKeyFromStringWithEncoding(
                                                                                              [key description], stringEncoding)];
        } else {
            [mutablePairs
             addObject:[NSString stringWithFormat:
                        @"%@=%@",
                        GTSDKAFPercentEscapedQueryStringKeyFromStringWithEncoding(
                                                                                  [key description], stringEncoding),
                        GTSDKAFPercentEscapedQueryStringValueFromStringWithEncoding(
...
static NSString *const kGTSDKAFCharactersToBeEscapedInQueryString = @":/?&=;+!@#$()',*";

static NSString *
GTSDKAFPercentEscapedQueryStringKeyFromStringWithEncoding(NSString *string,
                                                          NSStringEncoding encoding)
{
    static NSString *const kGTSDKAFCharactersToLeaveUnescapedInQueryStringPairKey = @"[].";

    return (__bridge_transfer NSString *)CFURLCreateStringByAddingPercentEscapes(
                                                                                 kCFAllocatorDefault, (__bridge CFStringRef)string,
                                                                                 (__bridge CFStringRef)kGTSDKAFCharactersToLeaveUnescapedInQueryStringPairKey,
                                                                                 (__bridge CFStringRef)kGTSDKAFCharactersToBeEscapedInQueryString,
                                                                                 CFStringConvertNSStringEncodingToEncoding(encoding));
}

static NSString *
GTSDKAFPercentEscapedQueryStringValueFromStringWithEncoding(NSString *string,
                                                            NSStringEncoding encoding)
{
    return (__bridge_transfer NSString *)CFURLCreateStringByAddingPercentEscapes(
